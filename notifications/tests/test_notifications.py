...
from notifications.utils import render_notification_template, send_notification
...
def test_notification_template_rendering(notification_template):
    ...
    rendered = render_notification_template(NotificationType.EVENT_CREATED, context, 'en')
    ...
    rendered = render_notification_template(NotificationType.EVENT_CREATED, context, 'fi')
    assert len(rendered) == 3
...
def test_notification_sending(notification_template):
    ...
    send_notification('foo@bar.fi', NotificationType.EVENT_CREATED, context)
